...
const DOCK_H = 72; // safe-ish (your DockArea is ~50-72 depending); adjust if needed
...
const computeSpawn = ({ home, size, spawnRef }) => {
  ...
  if (home && typeof home.ax === "number" && typeof home.ay === "number") {
    ...
    const baseX = VIEW_PADDING + clamp(home.ax, 0, 1) * Math.max(0, usableW);
    const baseY =
      VIEW_PADDING +
      SYSTEM_BAR_H +
...
export default function useWindowManager(initialWindows = DEFAULT_WINDOWS) {
  ...
  const restoreWindow = useCallback((id) => {
    setWindows((prev) => {
      ...
      const nextState = w.prevState === "fullscreen" ? "fullscreen" : "window";
  ...
  const minimizedWindows = useMemo(() => {
    // NOTE: you currently keep minimized windows in dock even when restored.
    // If you want them to STAY in dock until closed, change this filter logic accordingly.
